 ...
 */function deltaEOK(_,t){const[o,n,a]=_,[i,e,l]=t,r=o-i,u=n-e,s=a-l;return Math.sqrt(r**2+u**2+s**2)}function binarySearchGamut(_,t,o){let n=0,a=_[1];const i=_;for(;a-n>1e-4;){const _=clip(t(i));deltaEOK(OKLCH_to_OKLab(i),OKLCH_to_OKLab(o(_)))-.02<1e-4?n=i[1]:a=i[1],i[1]=(a+n)/2}return clip(t([...i]))}
/**
 ...
 */function multiplyMatrices(_,t){const o=_.length;let n,a;n=Array.isArray(_[0])?_:[_],a=Array.isArray(t[0])?t:t.map((_=>[_]));const i=a[0].length,e=a[0].map(((_,t)=>a.map((_=>_[t]))));let l=n.map((_=>e.map((t=>Array.isArray(_)?_.reduce(((_,o,n)=>_+o*(t[n]||0)),0):t.reduce(((t,o)=>t+o*_),0)))));return 1===o&&(l=l[0]),1===i?l.map((_=>_[0])):l}
/**
