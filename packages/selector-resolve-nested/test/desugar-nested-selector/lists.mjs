...
test('list - simple', async () => {
	assert.equal(
		desugarNestedSelector(parse('.foo, fooz'), parse('.bar')).toString(),
		'.bar .foo,.bar  fooz',
	);
});
...
test('list - list', async () => {
	...
	assert.equal(
		desugarNestedSelector(parse('.foo, .fooz, .foos'), parse('.bar, .baz, .bas')).toString(),
		'.bar .foo, .baz .foo, .bas .foo,.bar  .fooz, .baz  .fooz, .bas  .fooz,.bar  .foos, .baz  .foos, .bas  .foos',
	);

	assert.equal(
		desugarNestedSelector(parse('& & .foo, & & .fooz, & & .foos'), parse('.bar > :hover, .baz > :hover, .bas')).toString(),
