...
export function contrastColor(colorMixNode: FunctionNode, colorParser: ColorParser): ColorData | false {
	...
	backgroundColor.channels = XYZ_D50_to_sRGB_Gamut(colorData_to_XYZ_D50(backgroundColor).channels);
	backgroundColor.colorNotation = ColorNotation.sRGB;
	...
	const contrastWhite = contrastRatio(backgroundColor.channels, [1, 1, 1]);
	const contrastBlack = contrastRatio(backgroundColor.channels, [0, 0, 0]);
