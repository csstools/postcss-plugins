...
const creator: PluginCreator<pluginOptions> = (opts?: pluginOptions) => {
	...
	if (!directionValues.includes(options.blockDirection)) {
		throw new Error(`[postcss-logical-overscroll-behavior] "blockDirection" must be one of ${directionValues.join(', ')}`);
	}

	if (!directionValues.includes(options.inlineDirection)) {
		throw new Error(`[postcss-logical-overscroll-behavior] "inlineDirection" must be one of ${directionValues.join(', ')}`);
	}
	...
	if (!allAxesCovered) {
		throw new Error('[postcss-logical-overscroll-behavior] "blockDirection" and "inlineDirection" must be on separate axes');
	}
	...
	return {
		...
		Declaration: {
			'overscroll-behavior-block': makeTransform(transformAxes(directionConfig)),
			'overscroll-behavior-inline': makeTransform(transformAxes(directionConfig)),
		},
	};
};
