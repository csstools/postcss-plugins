...
var cssTokenizer = require('@csstools/css-tokenizer');
var cssParserAlgorithms = require('@csstools/css-parser-algorithms');
    ...
    prepare() {
      ...
      return {
        Declaration(decl, {
          ...
          const toDir = path.posix.parse(path.posix.resolve(to)).dir;
          const from = (_decl$source = decl.source) == null ? void 0 : _decl$source.input.from;
          if (!from) {
            return;
          }
          const fromDir = path.posix.parse(path.posix.resolve(from)).dir;
          const fromEntryPointDir = path.posix.parse(path.posix.resolve(fromEntryPoint)).dir;
          if (!URL_FUNCTION_CALL.test(decl.value)) {
            return;
          }
          const componentValuesList = cssParserAlgorithms.parseCommaSeparatedListOfComponentValues(cssTokenizer.tokenize({
            css: decl.value
          }));
          const modifiedComponentValuesList = cssParserAlgorithms.replaceComponentValues(componentValuesList, componentValue => {
            if (cssParserAlgorithms.isTokenNode(componentValue) && componentValue.value[0] === cssTokenizer.TokenType.URL) {
            ...
            if (cssParserAlgorithms.isFunctionNode(componentValue) && URL_FUNCTION_NAME.test(componentValue.getName())) {
              for (const x of componentValue.value) {
                if (cssParserAlgorithms.isWhitespaceNode(x) || cssParserAlgorithms.isCommentNode(x)) {
                  continue;
                }
                if (cssParserAlgorithms.isTokenNode(x) && x.value[0] === cssTokenizer.TokenType.String) {
          ...
          const modifiedValue = cssParserAlgorithms.stringify(modifiedComponentValuesList);
